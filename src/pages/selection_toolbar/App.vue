...
const handleCustomPrompt = async (promptName) => {
  await runAction(async (text) => {
    ...
    if (!ready) return
    await WindowService.selectionToolbarBlur()
    await AIService.streamCustomPrompt(text, promptName, t('selectionToolbar.chinese'))
...
.mini-icon.active {
  ...
  transition: transform var(--fy-duration-normal) var(--fy-ease-spring),
  box-shadow var(--fy-duration-normal) ease,
  background var(--fy-duration-normal) ease;
}
...
.magic-icon {
  ...
  transition: transform var(--fy-duration-normal) ease;
}
...
.pop-enter-active {
  transition: opacity var(--fy-duration-slow) ease,
  transform 0.4s var(--fy-ease-spring);
}
...
.toolbar-button {
  ...
  cursor: pointer;
  font-size: 18px;
  transition: all var(--fy-duration-normal) cubic-bezier(0.2, 0.8, 0.2, 1);
  display: flex;
  align-items: center;
  justify-content: center;
  position: relative;
...
/* 按钮背景使用语义化变量 + 主题适配 */
.translate-btn {
  --btn-bg: var(--fy-success-bg);
  --btn-bg-hover: var(--fy-success-bg-hover, rgba(74, 222, 128, 0.22));
...
.explain-btn {
  --btn-bg: var(--fy-accent-bg);
  --btn-bg-hover: var(--fy-accent-bg-hover);
  color: var(--fy-accent);
  background: var(--btn-bg);
}
.explain-btn:hover {
...
.copy-btn {
  --btn-bg: var(--fy-warning-bg);
  --btn-bg-hover: var(--fy-warning-bg-hover, rgba(251, 191, 36, 0.22));
...
.search-btn {
  --btn-bg: var(--fy-bg-overlay);
  --btn-bg-hover: var(--fy-bg-hover);
  color: var(--fy-info);
  background: var(--btn-bg);
}
.search-btn:hover {
